...
export default class LandingPage extends Component {
    ...
    readData = () => {
        ...
        Axios.get("/fileRead").then(res => {
            context.setState({
                envVariablesCount: Object.keys(res.data.pipeline.environment).length ? Object.keys(res.data.pipeline.environment).length : 1,
                entireJSON: res.data,
                savedData: res.data.pipeline.environment != undefined ? res.data.pipeline.environment : {},
                agentName: res.data.pipeline.agent != undefined ? res.data.pipeline.agent.name : ""
            })
    ...
    handleSave = () => {
        ...
        Axios.post("/fileWrite",entireJSON).then(res => {
            if(res.status === 200){
    ...
    handleKeyChange = (e, index) => {
        ...
        if(prevIndex === index  && Object.keys(savedData).length) {
            if (prevKey !== e.target.value) {
                Object.defineProperty(savedData, e.target.value,
        ...
        return(
            <React.Fragment>
                 ...
                 <Form className="agent-form" style={{marginLeft:"25%",width: "50%", marginTop: "4%"}}>
                    ...
                    <Form.Field>
                        <label style={{fontSize:'110%'}}>Environment</label>
                    </Form.Field>
                    {this.envVariablesDisplay()}
                    <Form.Field>
                        <Icon  name="add circle" position="right" size="large" style={{float:"right"}} onClick={() => {
                            this.setState({
                    ...
                    <center>
                    <Button
                    style={{ marginBottom: '4%', marginTop: '4%', letterSpacing: '1px', boxShadow: '3px 3px 11px 0 #9a9a9a'}}
                    color='black'
                    content="Proceed"
