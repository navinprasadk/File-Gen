...
export default class Editor extends Component {
    ...
    state={updatedProperties:[], currentTool: "", selectedTools: [], entireJSON: {}, stages:[], stageCount : 1}
    ...
    handleChangeValue = (e, currentTool, key) => {
        ...
        console.log("key",key);
         ...
         let currentProp = currentStageVal[this.props.activeStageNumber]["steps"] != undefined ? currentStageVal[this.props.activeStageNumber]["steps"] : [];
         ...
         context.setState({
             ...
             console.log("updatedValue",JSON.stringify(context.state.updatedProperties))
         })
    }
    ...
    updateData = () => {
        ...
        console.log("datasdsdsd",JSON.stringify(this.state.updatedProperties));
    ...
    componentWillReceiveProps = (nextProps) => {
        console.log("selectedTools",nextProps.selectedTools);
        this.setState({
 ...
 render(){
    let currentTool = this.state.currentTool != undefined ? this.state.currentTool.toLowerCase().replace(" ", "_") : "";
  
    let keys = currentTool ? Object.keys(properties[currentTool]) : [];

     return(
        <React.Fragment>
            { this.state.currentTool.length ? 
            <div>
            <p className='Editor_title'>{this.state.currentTool}</p>
            {keys.length ? keys.map(key => {
                ...
                <div>
                    ...
                    <Input
                        ...
                        onChange={(e) => {this.props.handleChangeValue(e, currentTool, key)}}
                    />
                </div>
                )
            }) : null}
            {/* {keys.length ? <Button size='tiny' color='black' className='Editor_save_button' onClick={this.updateData}>SAVE</Button> : null} */}
            </div> : <center className='Editor_empty_message'><p>No selected Tools</p></center>}
        </React.Fragment>
     );
 }
}
