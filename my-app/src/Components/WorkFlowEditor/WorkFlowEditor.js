        ...
        <div className="stage-container">
          <div className="header-content">
            ...
            <Icon
              ...
              name="remove circle"
              onClick={() => {
                this.props.deleteStage(this.props.stageNumber);
              }}
            />
          </div>
            ...
            <div>
              {this.props.selectedTools.length !== 0 ? (
                this.props.selectedTools.map((item, index) => {
                  return (
                    <Segment
                      className="NonEmpty_work_flow_editor_item_wrapper"
                      onClick={() => {
                        this.props.updateCurrentTool(item);
                      }}
                    >
                      <p className="NonEmpty_work_flow_editor_item">
                        {item.includes("_") ? item.replace("_", " ") : item}
                      </p>
                      <Icon
                        name="close"
                        className="NonEmpty_work_flow_editor_icon"
                        onClick={() => {
                          this.delete(index, this.props.stageNumber);
                        }}
                      />
                    </Segment>
                  );
                ...
                <div className="Empty_work_flow_editor">
                  <center>
                    <p className="Empty_work_flow_editor_message">
                      Drag and drop the components to here
                    </p>
                  </center>
                </div>
              )}
